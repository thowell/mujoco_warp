...
class Data:
  ...
  Attributes:
    ...
    vert: vertices in EPA polytope in Minkowski space           (nconmax, 5 + MJ_CCD_ITERATIONS)
    vert1: vertices in EPA polytope in geom 1 space             (nconmax, 5 + MJ_CCD_ITERATIONS)
    vert2: vertices in EPA polytope in geom 2 space             (nconmax, 5 + MJ_CCD_ITERATIONS)
    face: faces of polytope represented by three indices        (nconmax, 6 + 3 * MJ_CCD_ITERATIONS)
    face_pr: projection of origin on polytope faces             (nconmax, 6 + 3 * MJ_CCD_ITERATIONS)
    face_norm2: face_pr * face_pr                               (nconmax, 6 + 3 * MJ_CCD_ITERATIONS)
    face_index: index of face in polytope map                   (nconmax, 6 + 3 * MJ_CCD_ITERATIONS)
    map: status of faces in polytope                            (nconmax, 6 + 3 * MJ_CCD_ITERATIONS)
    horizon: index pair (i j) of edges on horizon in polytope   (nconmax, 3 * 2 * MJ_CCD_ITERATIONS)
  ...
  vert: wp.array2d(dtype=wp.vec)
  vert1: wp.array2d(dtype=wp.vec3)
  vert2: wp.array2d(dtype=wp.vec3)
  face: wp.array2d(dtype=wp.vec3i)
  face_pr: wp.array2d(dtype=wp.vec3)
  face_norm2: wp.array2d(dtype=float)
  face_index: wp.array2d(dtype=int)
  map: wp.array2d(dtype=int)
  horizon: wp.array2d(dtype=int)
