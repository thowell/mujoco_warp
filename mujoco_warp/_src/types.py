...
class Data:
  ...
  Attributes:
    ...
    vert: vertices in EPA polytope in Minkowski space           (nconmax, 5 + CCDiter)
    vert1: vertices in EPA polytope in geom 1 space             (nconmax, 5 + CCDiter)
    vert2: vertices in EPA polytope in geom 2 space             (nconmax, 5 + CCDiter)
    face: faces of polytope represented by three indices        (nconmax, 6 + 3 * CCDiter)
    face_pr: projection of origin on polytope faces             (nconmax, 6 + 3 * CCDiter)
    face_norm2: face_pr * face_pr                               (nconmax, 6 + 3 * CCDiter)
    face_index: index of face in polytope map                   (nconmax, 6 + 3 * CCDiter)
    map: status of faces in polytope                            (nconmax, 6 + 3 * CCDiter)
    horizon: index pair (i j) of edges on horizon in polytope   (nconmax, 3 * 2 * CCDiter)
