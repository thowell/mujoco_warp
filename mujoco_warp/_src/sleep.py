...
@wp.func
def _is_static_awake(
  # In:
  body_treeid_val: int,
  body_rootid_val: int,
  body_mocapid_val: int,
  flg_staticawake: int,
) -> int:
  """Checks if body is static/mocap and returns its sleep state, else -1."""
  if body_treeid_val < 0:
    if body_mocapid_val >= 0:
      return SleepState.AWAKE
    else:
      return SleepState.AWAKE if flg_staticawake != 0 else SleepState.STATIC
  return -1
...
def _update_sleep_bodies(
  ...
  mocap = body_mocapid[root] if root >= 0 else -1

  # check static
  state = _is_static_awake(tree, root, mocap, flg_staticawake)
  if state == -1:
